...
- **codex（GPT-5.6 Sol）**: Claude Codeホストから使う外部パネリストとして、現行の分離フラグ込みで実機E2E済み（2026-07-13、codex-cli 0.144.1、`-m gpt-5.6-sol` 固定）。**既定パネルに参加**（外すのは `QUORUM_ENABLE_CODEX` を空文字/`0`/`false` にした時のみ）。**Codexホストでは外部 `run_codex.sh` を除外**し、ネスト・再帰を防ぐ。
...
- 「sh で叩く＝サブスク不可」は誤り。**claude / codex / gemini / grok はログイン方式なら各サービスのプラン・無料枠で動かせる**（条件は各サービス契約による）。
...
さらに **`rules/settings-env.json` の環境変数を各PCの `~/.claude/settings.json` の `env` にマージ**する（現在は `QUORUM_ENABLE_CODEX=1`＝既定オンの明示）。codex は run_codex.sh 側でも**未設定=オン**なのでこのマージは冗長な保険だが、マージは**未設定キーのみ**——そのPCで明示した値は上書きされないので、特定PCで codex を無効にしたければ settings.json で `"QUORUM_ENABLE_CODEX": ""` と空文字を置けばよい（再インストールでも保持される）。codex 未導入のPCでは変数が立っていても `--check` が CLI 不在で弾くため無害。壊れた settings.json には触れない（警告してスキップ）。

**各PC固有で別途必要なもの**（秘密情報のため Git には入れない・入れてはいけない）:
- 使いたいモデルCLIの導入と**認証**:
  - claude（Anthropic）: `claude auth login`（Codexホストでは既定で外部パネルに参加。外すのは `QUORUM_ENABLE_CLAUDE=0`）
  - codex（OpenAI）: `codex login`（既定で参加。外すのは `QUORUM_ENABLE_CODEX` を空文字/`0`/`false` に）
...
| `QUORUM_PANEL_SIZE` | 3 | 目標パネル数。利用可能ならClaudeホストは opus/codex/grok、Codexホストは codex-native/claude/grok。欠員はホストのネイティブ枠で補完 |
...
| `QUORUM_ENABLE_CLAUDE` | **オン（未設定=1）** | 空文字・`0`・`false` でCodexホストの外部Claudeを外す。Claudeホストでは常に外部Claudeを除外 |
| `QUORUM_ALLOW_CLAUDE_API` | (未設定) | `ANTHROPIC_API_KEY` 検出時も外部Claudeを許可する明示スイッチ。`1`/`true`/`yes` のみ有効 |
| `QUORUM_ENABLE_CODEX` | **オン（未設定=1）** | 空文字・`0`・`false` でClaude Codeホストの外部codexをパネルから外す。Codexホストでは常に外部codexを除外 |
| `QUORUM_ENABLE_GEMINI` | (未設定) | `1` で gemini を既定パネルに復帰させる（`0`/`false`/空文字は無効扱い） |
