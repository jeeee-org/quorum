...
- **claude（Opus）**: Codexホストから使う外部パネリスト。`claude -p` をsafe-mode・ツールなし・セッション非保存・空CWDで隔離する。Claudeホストでは除外して再帰を防ぐ。Claude Code 2.1.207で最小E2E済み（2026-07-13）。
- **codex（GPT-5.6 Sol）**: Claude Codeホストから使う外部パネリストとして、現行の分離フラグ込みで実機E2E済み（2026-07-13、codex-cli 0.144.1、`-m gpt-5.6-sol` 固定）。**既定オフ（opt-in）。使うPCで `QUORUM_ENABLE_CODEX=1` にすると参加**。**Codexホストでは外部 `run_codex.sh` を除外**し、ネスト・再帰を防ぐ。
...
- `gemini-2.5-pro` は **無料枠では実用にならない**。AI Studio APIキー無料枠では `gemini-2.5-pro` が `limit: 0`（=構造的に不可。「retry in 30s」表示は誤解を招くが、待っても無料では通らない。要・課金アカウント紐付け）。**flash は無料枠で動作**（2026-06-17 実キー E2E 確認済）。∴ **無料枠キーの既定は `gemini-2.5-flash`**（`run_gemini.sh` の既定もこれに合わせた）。
  - 📌 2026-07-15 続報（quorum で確認）：`gemini-2.5-pro` の無料枠は 2026年4月に公式に恒久撤廃（上の `limit: 0` 観測が正式仕様として確定）。世代交代として `gemini-3.5-flash` が 2026-05-19 GA・無料枠対象（2.5-flash 系と同水準）。既定モデルを `gemini-3.5-flash` に更新（下記）。**`gemini-3.5-pro` は 2026-07-15 時点でもまだ一般提供されていない**（同日発表・「来月ロールアウト予定」のまま未GA。一部情報源では `gemini-3.1-pro` という別呼称も混在しており正式名称・仕様は未確定）。
...
> - ⚠️ **2026-07-15 続報**：「APIキーは別系統で影響なし」は今回も公式に再確認された（Gemini CLI→Antigravity移行の公式告知で「有償Gemini/Gemini Enterprise Agent Platform APIキー経由は継続サポート」と明言）。ただし**別件**として、GoogleはStandard API keyからサービスアカウント紐付きのAuthorization keyへの移行を進めており、**2026年9月にStandard key全般を拒否予定**。`GEMINI_API_KEY`/`GOOGLE_API_KEY` の環境変数名自体は変わらないが、**保存済みキーの種別確認とAuthorization keyへの切替が9月までに必要**（両方設定時は `GOOGLE_API_KEY` が優先される点にも注意）。
...
| `claude` | Claude Opus | `claude -p --safe-mode`（Codexホスト専用） | Claude.aiログイン=プラン利用枠 / `ANTHROPIC_API_KEY`=従量（既定ガードあり） |
| `codex` | GPT-5.6 Sol | `codex exec -m gpt-5.6-sol`（Claude Codeホスト専用） | ChatGPT ログイン=サブスク枠 / APIキー=従量 |
| `gemini` | Gemini | `gemini -p` | Googleログイン無料枠は**2026-06-18廃止**（→Antigravity CLI `agy`へ移行、本項目上部の警告参照） / APIキー=従量 |
| `grok` | Grok (xAI) | `grok -p`（CLI）/ xAI API（curl） | **サブスク枠**（SuperGrok/X Premium+・`grok login`）or APIキー=従量 |

- 「sh で叩く＝サブスク不可」は誤り。**claude / codex / grok はログイン方式なら各サービスのプラン・無料枠で動かせる**（条件は各サービス契約による）。**gemini のみ例外**：`gemini` CLIの個人向けログイン無料/Pro/Ultra枠は2026-06-18廃止済みのため、本表の「Googleログイン」経路は使えない（後継の `agy` はheadless未成熟でパネリスト不可、上部の警告参照）。gemini は事実上APIキー方式のみ。
- API キー方式（`ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GEMINI_API_KEY` / `XAI_API_KEY`）にすると従量課金になる。外部ClaudeはAPIキーを検出すると、`QUORUM_ALLOW_CLAUDE_API=1` がない限り参加しない。
...
`install.sh` は skills / commands / ランチャーに加えて、Claude Codeの `~/.claude/CLAUDE.md` へ既存トリアージ規則を、Codexの `$CODEX_HOME/AGENTS.md` へ **T1 → `$quorum` の接続規則だけ**をマーカーブロックとして注入する。Codexの分類正本は `claude-rules` に置き、quorum側では重複定義しない。新PCでは `claude-rules → quorum → cadence` の順で各 `install.sh` を実行するのを推奨する。`claude-rules` 未導入時は自動T1分類だけが利用不能で、ユーザーが明示する `$quorum` は利用できる。

さらに **`rules/settings-env.json` の環境変数を各PCの `~/.claude/settings.json` の `env` にマージ**する（現在は `QUORUM_ENABLE_CODEX/GROK/GEMINI` をすべて `"0"`＝**既定オフの明示**）。これにより新PCは何もしなければ外部ゼロ＝`opus×3` で動く。参加させたい外部は、そのPCの settings.json で該当キーを `"1"` にする（`codex`/`grok` なら `-m` 対応CLIと認証も必要）。マージは**未設定キーのみ**——PCで明示した値（`"1"` でも `"0"` でも）は再インストールで上書きされない。壊れた settings.json には触れない（警告してスキップ）。

**各PC固有で別途必要なもの**（秘密情報のため Git には入れない・入れてはいけない）:
- 使いたいモデルCLIの導入と**認証**:
  - claude（Anthropic）: `claude auth login`（Codexホストの外部パネル。既定オフ、`QUORUM_ENABLE_CLAUDE=1` で参加）
  - codex（OpenAI）: `codex login`（既定オフ、`QUORUM_ENABLE_CODEX=1` で参加）
  - grok（xAI/SuperGrok）: `curl -fsSL https://x.ai/cli/install.sh | bash` → `grok login`（既定オフ、`QUORUM_ENABLE_GROK=1` で参加）
  - gemini（任意）: `npm i -g @google/gemini-cli` → ログイン（既定オフ、`QUORUM_ENABLE_GEMINI=1` で参加）
...
CodexホストのオプトインenvはClaudeの `settings.json` からは継承されない。起動シェルで渡す（例: `QUORUM_ENABLE_GEMINI=1 codex`）か、シェル設定へexportする。
...
- **課金**: ランチャーは**対話モードの `claude`** を起動する。2026-07-13時点では `claude -p` もClaudeプランの利用枠を消費する（Anthropicの変更案は一時停止中）。ただし `ANTHROPIC_API_KEY` が環境にある場合はAPI従量になるため、ランチャーと外部Claude runnerはいずれも警告・ガードする。最新条件は [Anthropic公式案内](https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan) を確認する。
...
| `QUORUM_ENABLE_GROK` | **オフ（未設定=不参加）** | `1`/`true`/`yes` で grok を参加（`0`/`false`/`no`/空文字は無効扱い） |
| `QUORUM_ENABLE_GEMINI` | **オフ（未設定=不参加）** | `1` で gemini を参加させる（`0`/`false`/空文字は無効扱い） |
| `CLAUDE_MODEL` / `GROK_MODEL` / `GEMINI_MODEL` | `opus` / - / - | 各モデルIDの明示（公平比較用） |
