...
同じ問いを**互いにブラインドで独立並列**に複数の回答者へ投げ、**メインセッションの Claude（judge）が合意・矛盾・盲点まで突き合わせて**最終回答を書く（fan-out → judge → fuse）。
...
>
> **Fable 5 登場後の再定義（2026-07）**：「Fable 級を合成する」という当初の物語は本物の Fable 5 が使える今、役目を終えた。残った——むしろ最初から本体だった——価値は、**単一ベンダーでは原理的に消せない系統的盲点への保険**。同一モデルを N 体走らせても全員が同じ場所で転ぶが、別ベンダーのモデル（GPT/Grok/Gemini）は訓練データも検索経路も癖も違うため、相関しない誤りを相殺できる。∴ **judge にはセッションで使える最強モデル（例 Fable 5）を座らせ、パネルには安価な Claude（opus 明示指定）と異種ベンダーを並べる**。深さは judge、幅はパネルの分担（`references/panel.md`）。
...
| `QUORUM_ENABLE_CODEX` | **1**（install.sh が settings.json へマージ） | codex をパネルに含める。無効化したい PC は settings.json の env で空文字に（マージは未設定キーのみで、明示値は保持される） |
