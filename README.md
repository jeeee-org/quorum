...
- **codex（GPT-5.6 Sol）**: Claude Codeホストから使う外部パネリストとして実機検証済み（codex-cli 0.144.1、`-m gpt-5.6-sol` 固定）。**既定パネルに参加**（外すのは `QUORUM_ENABLE_CODEX` を空文字/`0`/`false` にした時のみ）。**Codexホストでは外部 `run_codex.sh` を除外**し、ネスト・再帰を防ぐ。
...
| 変数 | 既定 | 効果 |
|---|---|---|
| `QUORUM_PANEL` | (未設定) | パネルの明示指定（カンマ/空白区切り multiset。タブ・改行も可。例 `opus,opus,codex,grok`）。指定時は検出・`--check`・補完を飛ばしてそのまま採用（全枠が実行時失敗したら fuse せず中断＝SKILL 規定）。Codexホストの外部codex禁止だけは上書き不可 |
| `QUORUM_PANEL_SIZE` | 3 | 目標パネル数。既定パネルは opus/codex/grok の3枠。欠員は **opus → codex → grok の優先順**で可用な補完枠が埋める（Claudeホストでは実質 opus、Codexホストでは codex-native）。distinct が目標超の時だけトリムし明示 |
