...
- **codex（GPT-5.6 Sol）**: Claude Codeホストから使う外部パネリストとして実機検証済み（codex-cli 0.144.1、`-m gpt-5.6-sol` 固定）。**既定パネルに参加**（外すのは `QUORUM_ENABLE_CODEX=""` の空文字オプトアウト時のみ）。**Codexホストでは外部 `run_codex.sh` を除外**し、ネスト・再帰を防ぐ。
...
- 使いたいモデルCLIの導入と**認証**:
  - codex（OpenAI）: `codex login`（既定で参加。外すのは `QUORUM_ENABLE_CODEX=""`）
...
| 変数 | 既定 | 効果 |
|---|---|---|
| `QUORUM_PANEL` | (未設定) | パネルの明示指定（カンマ/空白区切り multiset。例 `opus,opus,codex,grok`）。指定時は検出・`--check`・補完を飛ばしてそのまま採用。特定バックエンドの増員・固定用。Codexホストの外部codex禁止だけは上書き不可 |
...
| `QUORUM_ENABLE_CODEX` | **オン（未設定=1）** | 空文字でClaude Codeホストの外部codexをパネルから外す。Codexホストでは常に外部codexを除外 |
