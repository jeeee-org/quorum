...
| 変数 | 既定 | 効果 |
|---|---|---|
| `QUORUM_PANEL_SIZE` | 4 | **目標パネル数**。既定パネルは grok/opus/codex + opus補完の4枠（**gemini は既定で外し opus 補完**、入れるのは `QUORUM_ENABLE_GEMINI=1` 時のみ）。使えない枠も**独立 opus 実行で補完**（detect_panel.sh が自動バックフィル）。distinct が目標超の時だけトリムし明示（無音切り捨てなし） |
