...
3. 出力の各行をそのままパネリストにする（`opus` 行は Task で独立 spawn、**model は opus を明示指定**（セッションモデルを継承させない）、複数なら opus#1/opus#2… と区別。非 opus 行は `run_<name>.sh`）。既定パネルは opus/codex/grok の3枠（gemini は `QUORUM_ENABLE_GEMINI=1` でオプトイン、codex は `QUORUM_ENABLE_CODEX=""` でオプトアウト）。使えない枠・無効化した枠は opus で補完される。行数が目標超の時だけ優先順位でトリムし、落としたものを明示。
