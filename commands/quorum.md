...
2. `~/.claude/skills/quorum/scripts/detect_panel.sh` を実行して利用可能なバックエンドを検出する。
3. 利用可能なものを**全部**パネリストに使う（最低2回は独立実行。`opus` のみなら Opus 4.8 を2回。`QUORUM_MAX_PANELISTS` 超過分は明示して除外）。
