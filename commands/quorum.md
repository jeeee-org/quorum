...
3. 出力の各行をそのままパネリストにする（`opus` / `fable` 行は Task で独立 spawn、**model をその名前で明示指定**（セッションモデルを継承させない）、複数なら opus#1/opus#2… と区別。`fable` 行はユーザーの呼びかけ時のみ＝spawn 前に1行宣言し fable_calls.log に追記。それ以外の行は `run_<name>.sh`）。外部は既定オフ（opt-in）なので、何も有効化していないPCの既定パネルは opus×3。`QUORUM_ENABLE_CODEX=1`・`QUORUM_ENABLE_GROK=1` を立てたPCでは利用可能なら opus/codex/grok の3枠。distinct backendで足りない枠は独立opusで補完する。行数が目標超の時だけ優先順位でトリムし、落としたものを明示。
