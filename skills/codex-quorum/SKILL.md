...
- Codexホストでは `run_codex.sh` を絶対に呼ばない。外部Codexのネストと quorum 再帰を避ける。
- 特定バックエンドの増員・固定が必要な時は `QUORUM_PANEL="codex-native,codex-native,grok"` の明示指定を detect_panel.sh に渡す（指定時は検出・補完を飛ばす。外部 codex は明示でも拒否される）。
...
5. 各成功回答の全文を匿名の `answer_<label>.md` に保存する。パネリスト間で回答を共有しない。timeout・空応答・失敗は dropped として記録し、成功分で続行する。
