...
- **同じ問いの2巡目（差し戻し後の再レビュー）は、前巡の指摘を ID つきの本文で貼り「閉じたか」を問う形に組む**（`references/context-packing.md`「2巡目（再レビュー）は…」）。回答は「①指摘ごとの判定 ②新しい欠陥 ③総合判定」の3分割で要求する。**前巡の `judge.md` を渡す枠と渡さない枠を混ぜない**——2巡目は「リポを読める枠だけが前巡の成果物を読める」形に黙ってなりやすく、同一プロンプトの規律が破れる。
- **コード差分を pack に貼る時は、範囲を明示して貼る前に検算する**：`<レビュー対象が分岐した commit>..<HEAD>` を明示指定し（`~N` / `HEAD~N` は**マージコミットで親が2つある**ため直感と食い違う）、`git log --oneline <範囲>` の出力がレビュー対象と一致することを目視してから貼る。触ったファイルの一覧も併記する。**材料が誤っていればパネリストの推論が正しくても結論は誤り、全員が同じ材料を見るので多数決では検出できない**（`references/context-packing.md`「コードの差分を貼るときは範囲を検算してから貼る」）。
...
- **`invalid_response` が出た枠は opus で1回だけ補完する**（幅が黙って縮退するのを防ぐ）。同じ `$PROMPT` を独立 opus に投げ直し、`opus#N（<backend> 枠の補完）` として mapping.txt と監査証跡に明示する。補完しない判断（同型の指摘が既に出揃っている等）をした時も、**パネルが N 体→M 体に縮んだことを監査証跡に書く**。判定材料が減った事実を judge の結論より先に読める場所へ置く。
  - `--backend` を渡していると、**同じ backend が2回連続で `invalid_response` になった時に stderr へ警告**が出る（`QUORUM_INVALID_WARN` 既定2・0で無効。状態は `$QUORUM_STATE_DIR/invalid.tsv`）。これは detect_panel の連続欠席警告では捕まらない型——`--check` は通るのに毎回メタ応答しか返らない状態——なので、出たら**その run では補完し、縮退した事実を監査証跡に明記し、ユーザーに伝える**。ただし**恒久故障と断定してパネルの枠から外さない**：6 run 連続で欠席した後に 17KB の実質回答を返し、しかも**単独でしか出ない高重大度の指摘を4件**出した実例がある（IMPROVEMENTS 2026-08-18）。この型の不調は run ごとに揺れるので、外すと「効いているパネリスト」を失う。警告が出たら疑うのは backend ではなく**認証・CLI 版・オプション**（`--help` を読み直す）。
  - なお `run_grok.sh` は、正常終了かつ回答が閾値未満なら**run 側で1回だけ自動的に投げ直す**（`QUORUM_GROK_RETRY=0` で無効）。SKILL 側の補完は「そのリトライでも回復しなかった枠」に対する二段目の受け皿。
- **ネイティブ枠（opus / fable）が異常終了した場合も1回だけ再投入する**。`check_answer.sh` は外部バックエンドのファイル出力しか見ないので、**サブエージェントの異常終了（API エラー・接続断など）はこの検査の射程外**——判定の穴になる。再投入も落ちたら、生存しているパネリスト数を監査証跡に明記する。2026-08-18 に、opus 枠の `ECONNRESET` と grok の欠席が重なって**一時的に生存1体**まで縮退した実例がある。
- **fan-out の回収時に `git status` で作業ツリーの汚れを確認する**。リポを読める枠は書き込みもできるため、意図せず元 clone や worktree を書き換えていることがある（`references/panel.md`「リポを読める枠には、書き込みの可否と作業場所を必ず書く」）。汚れていたら、それがパネリストの副作用か自分の作業かを切り分けてから judge に進む。
...
- **Consensus**（合意点。**同じ場所を指しただけで束ねない**——根拠・機序・影響の向きが揃うかを1段掘り、食い違ったら Contradictions へ移す）
- **Contradictions**（食い違い。どちらがより確からしいか根拠つきで判定）
  - **「スコープ外の変更がある」「材料の版が違う」型の指摘は、採否の前に judge が範囲・版を取り直して検算する**（judge_rubric「judge 自身が確かめる」）。
