...
新しいモデルを足すには、規約に従う `run_<name>.sh`（`--check` で可用判定＋stdin→stdout）を置くだけ。detect も fan-out も無改修で対応する。**プロンプトを argv に展開しない**（stdin かファイル渡し）——単一引数長の上限 ≒128KB を超えると exec が失敗して**無言の空応答**になり、大型 pack でそのバックエンドだけが構造的に欠席する。
...
- **回収後の軽量検査（監査記録・自動棄却しない）**：外部バックエンドの各回答は保存後に `bash ~/.claude/skills/quorum/scripts/check_answer.sh --backend <backend> "$RUN_DIR/answer_<label>.md" "$RUN_DIR/answer_<label>.err"` に通す（第2引数の stderr は任意だが、渡すと空応答の原因まで名指しできる。既知パターン以外でも `empty:<stderr先頭行>` として転記されるので、認証切れ・レート制限などの恒久故障を「今回は欠席」で流さずに済む）。`invalid_response:<reason>`（空・argv 上限超過・極端な短文）が出たら `$RUN_DIR/checks.txt` に `<label>\t<verdict>` を追記し、judge はその回答を**実質回答なしの疑い**として精査する——実質回答があるなら採用理由を、無いなら dropped 判定を監査証跡に明記する。**exit 0・非空でも成功と見なさない**（エージェント型CLIは途中報告だけで正常終了し得る）。※現段階は監査記録のみ。誤棄却が無いことを確認できたら run 側の最小バイト数ゲートへ格上げする（IMPROVEMENTS 2026-07-13）。格上げ可否の判断材料は `bash ~/.claude/skills/quorum/scripts/checks_summary.sh` で出せる（過去 run の checks.txt を横断集計し、閾値に近い＝誤棄却の疑いが濃い順に並べる）。
- **`invalid_response` が出た枠は opus で1回だけ補完する**（幅が黙って縮退するのを防ぐ）。同じ `$PROMPT` を独立 opus に投げ直し、`opus#N（<backend> 枠の補完）` として mapping.txt と監査証跡に明示する。補完しない判断（同型の指摘が既に出揃っている等）をした時も、**パネルが N 体→M 体に縮んだことを監査証跡に書く**。判定材料が減った事実を judge の結論より先に読める場所へ置く。
  - `--backend` を渡していると、**同じ backend が2回連続で `invalid_response` になった時に stderr へ警告**が出る（`QUORUM_INVALID_WARN` 既定2・0で無効。状態は `$QUORUM_STATE_DIR/invalid.tsv`）。これは detect_panel の連続欠席警告では捕まらない型の恒久故障——`--check` は通るのに毎回メタ応答しか返らない状態——なので、出たら**「恒久故障の疑い」として監査証跡に明記し、ユーザーに伝える**（IMPROVEMENTS 2026-08-11 / 08-12 の grok）。
  - なお `run_grok.sh` は、正常終了かつ回答が閾値未満なら**run 側で1回だけ自動的に投げ直す**（`QUORUM_GROK_RETRY=0` で無効）。SKILL 側の補完は「そのリトライでも回復しなかった枠」に対する二段目の受け皿。
...
- **書く位置**: **ファイル末尾へ追記する**（並びは古い順。先頭へ差し込まない）。複数PCの clone が末尾追記どうしを素直に merge できる形を保つため——並べ替えると全項目 conflict になり、その回の実質的な追記まで巻き込んで失う（IMPROVEMENTS 2026-08-09）。
