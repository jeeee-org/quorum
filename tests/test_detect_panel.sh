...
t "grok 可用 → opus+grok+opus 補完で3枠" \
  "$(printf 'opus\ngrok\nopus')" \
  "$(bash "$TMP/one/detect_panel.sh")"

mk_env "$TMP/default" codex:0 grok:0
t "codex+grok 可用 → 既定パネルは opus/codex/grok の3枠ちょうど" \
  "$(printf 'opus\ncodex\ngrok')" \
  "$(bash "$TMP/default/detect_panel.sh")"
...
QUORUM_PANEL_SIZE=abc bash "$TMP/size/detect_panel.sh" >/dev/null 2>&1
t "QUORUM_PANEL_SIZE の非整数はexit 2" "2" "$?"

QUORUM_PANEL_SIZE=0 bash "$TMP/size/detect_panel.sh" >/dev/null 2>&1
t "QUORUM_PANEL_SIZE=0 はexit 2" "2" "$?"
...
# QUORUM_PANEL 明示指定（増員・固定用）
mk_env "$TMP/explicit" grok:1
t "QUORUM_PANEL は --check・補完を飛ばしてそのまま出力（増員可）" \
  "$(printf 'opus\nopus\ngrok')" \
  "$(QUORUM_PANEL="opus,opus,grok" bash "$TMP/explicit/detect_panel.sh")"

t "QUORUM_PANEL は空白区切りも受ける" \
  "$(printf 'grok\ngrok')" \
  "$(QUORUM_PANEL="grok grok" bash "$TMP/explicit/detect_panel.sh")"

t "QUORUM_PANEL はタブ・改行区切りも受ける（黙殺しない）" \
  "$(printf 'opus\ngrok\ngrok')" \
  "$(QUORUM_PANEL="$(printf 'opus\tgrok\ngrok')" bash "$TMP/explicit/detect_panel.sh")"

QUORUM_PANEL="opus,nonexistent" bash "$TMP/explicit/detect_panel.sh" >/dev/null 2>&1
t "QUORUM_PANEL の不明なバックエンドはexit 2" "2" "$?"
