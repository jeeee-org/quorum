...
# temp の CLAUDE_CONFIG_DIR / CODEX_HOME / BIN_DIR に対して実行するので実環境には触れない。
set -uo pipefail
...
CFG="$TMP/claude"; CODEX="$TMP/codex"; BIN="$TMP/bin"
...
# 1回目のインストール
CLAUDE_CONFIG_DIR="$CFG" CODEX_HOME="$CODEX" BIN_DIR="$BIN" bash "$REPO/install.sh" >/dev/null 2>&1
...
CLAUDE_CONFIG_DIR="$CFG" CODEX_HOME="$CODEX" BIN_DIR="$BIN" bash "$REPO2/install.sh" >/dev/null 2>&1
